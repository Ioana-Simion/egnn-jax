...
Similarly to how the equivariant EGNN in \[5\] is made equivariant, we also introduce one layer where we preidct $\delta x$. We implemented two versions: One that calculates the position update and one version that takes velocity into account. 

$$\begin{align} 
x^pred &= x + \Phi(??????), \qquad \qquad \text{(Equation 12)}\\
x^pred &= x + velocity * \Phi(??????), \qquad \qquad \text{(Equation 13)}
\end{align}$$
