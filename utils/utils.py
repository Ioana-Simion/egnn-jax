...
def get_loaders(args: Namespace, transformer=False) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    if args.dataset == "qm9":
        ...
        if transformer:
            ...
            train_loader = GDataLoader(dataset[:num_train], batch_size=args.batch_size, shuffle=True, drop_last=True)
            val_loader = GDataLoader(dataset[num_train:num_train+num_val], batch_size=args.batch_size, drop_last=True)
            test_loader = GDataLoader(dataset[num_train+num_val:], batch_size=args.batch_size, drop_last=True)
    elif args.dataset == "charged":
