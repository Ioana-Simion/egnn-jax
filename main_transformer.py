...
def train_model(args, model, graph_transform, model_name, checkpoint_path):
    ...
    loss_fn = partial(l1_loss, model_fn=model.apply, task=args.task)
    update_fn = partial(update, loss_fn=loss_fn, opt_update=opt_update)
    eval_fn = partial(evaluate, loss_fn=loss_fn, graph_transform=graph_transform_fn, task=args.task)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_edge_encoders",
        action="store_true",
        help="Number of edge encoder blocks",
    )
    parser.add_argument(
        "--num_node_encoders",
        action="store_true",
        help="Number of node encoder blocks",
    )
    ...
    parser.add_argument(
        "--edge_input_dim",
        action="store_true",
        help="Dimension of the edge input",
    )
    parser.add_argument(
        "--node_input_dim",
        action="store_true",
    ...
    parser.add_argument("--model_name", type=str, default="transformer", help="model")
