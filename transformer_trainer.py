...
def update(params, edge_attr, node_attr, target, opt_state, rng, model_fn, opt_update):
    ...
    grads = jax.grad(mse_loss)(
        params, edge_attr, node_attr, target, dropout_rng, model_fn
    )
...
def train_model(args, model, model_name, checkpoint_path):
    ...
    params = model.init(
        init_rng, edge_inputs=init_edge_attr, node_inputs=init_node_attr
    )["params"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=32, help="Batch size (number of graphs)."
    )
