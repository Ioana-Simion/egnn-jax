...
def NbodyBatchTransform(
    ...
    def _to_invariant_transformer(
        ...
        (nodes, pos, edge_indices, vel, edge_attr), targets = _to_egnn(data)
        # invariant case add positions to mlp
        nodes = jnp.concatenate((nodes, pos), axis=1)
        return transformer_reshape(nodes, pos, edge_indices, vel, edge_attr, targets)

    def transformer_reshape(nodes, pos, edge_indices, vel, edge_attr, targets):
